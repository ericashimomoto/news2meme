...
def mean_square_singular_values(X, dims):
    ...
    mssv = torch.div(torch.mul(X, X).sum([1,2]),minimum)
    return mssv
...
def find_closest_subspace(ref_subspaces, ref_dims, input_subspaces, input_dims):
    ...
    for i in range(len(input_subspaces)):
        input = input_subspaces[i]
        ...
        # Get similarities
        dot = torch.bmm(torch.transpose(ref_subspaces, 1, 2), input_subspace_expand)
